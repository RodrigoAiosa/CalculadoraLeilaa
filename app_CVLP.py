...
def main():
    ...
    # --- SIDEBAR: CONFIGURAÇÕES E PERFIS ---
    st.sidebar.header("⚙️ Configurações")
    tipo = st.sidebar.selectbox("Tipo de Imóvel:", ["Casa", "Apartamento", "Terreno", "Gleba"])
    ...
    with st.container(border=True):
        ...
        taxas_arrematacao = v_lance * 0.08 # ITBI, Escritura, Leiloeiro
        custos_segura = (custo_fixo_mensal * meses) + (v_mensal * meses)
        total_investido_bolso = v_entrada + taxas_arrematacao + reforma + custos_segura

    with st.container(border=True):
        st.subheader("🏷️ Venda e Lucro")
        ...
        lucro_bruto = (v_venda - v_comis) - v_finan - total_investido_bolso
        v_ir = max(0.0, lucro_bruto * 0.15) # Estimativa IR 15%
        lucro_liquido = lucro_bruto - v_ir
        roi = (lucro_liquido / total_investido_bolso * 100) if total_investido_bolso > 0 else 0

        st.divider()
        res1, res2, res3 = st.columns(3)
        res1.metric("Total Investido (Do Bolso)", format_brl(total_investido_bolso))
        ...
        try:
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                df = pd.DataFrame([{
                    ...
                    "Tipo": tipo,
                    "Lance": v_lance,
                    "Investido": total_investido_bolso,
                    "Venda": v_venda,
    ...
    if excel_data:
        st.sidebar.download_button("📥 Baixar Relatório Excel", excel_data, f"leilao_{tipo}.xlsx")
